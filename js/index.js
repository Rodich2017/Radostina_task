;(function(global){
   var AddressBook = function( name,phone,ph_type,stat,email){
     return new AddressBook.init( name,phone,ph_type,stat,email);
   };
   ...
   AddressBook.prototype = {
     ...
     addNewContact:function( name,phone,ph_type,stat,email){
       this.data.push({
         name: name,
         phone: phone,
		 phone: ph_type,
   ...
   AddressBook.init=function( name,phone,ph_type,stat,email){
     var self = this;
     //set up the address book
     if(name || phone || ph_type || stat || email){
       self.addNewContact(name || "", phone||"", ph_type ||"", stat||"", email||"");
     }
...
form.addEventListener('submit', function(){
   if(!window.contactList){ //check if we already have a contact list
   window.contactList=$ab(form.person.value,form.phone.value,form.ph_type.value,form.stat.value,form.email.value);
  } else {
  //saves new values rather than deleting old ones as well
    contactList.addNewContact(form.person.value,form.phone.value,form.ph_type.value,form.stat.value,form.email.value);
  }
...
searchForm.addEventListener('submit', function(){
  ...
  if(results.length>0){
    ...
    for(var i = 0;i<results.length;i++){
      document.getElementById('results').innerHTML += '<div class="contact-item">Name:'+results[i].name+'<br>Phone:'+results[i].phone+'<br>Phone type:'+results[i].ph_type+'<br>Status:'+results[i].stat+'<br>Email:'+results[i].email+'</div><hr>';
    }
  } else{
  ...
  if(window.contactList){ //check if we already have a contact list
   ...
   var contacts = contactList.returnAll();
    ...
    if(contacts.length>0){
      ...
      document.getElementById('show-panel').innerHTML += '<div class="contact-item">Name:'+contacts[i].name+'<br>Phone:'+contacts[i].phone+'<br>Phone type:'+contacts[i].ph_type+'<br>Status:'+contacts[i].stat+'<br>Email:'+contacts[i].email+'</div><hr>';
      }
    }else{
